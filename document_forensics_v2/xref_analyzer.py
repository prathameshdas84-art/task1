"""
XREF Sequence Analyzer — Layer 6 (Extended)
Detects Canva-exported PDFs by checking for abnormal text object ordering.

UPGRADE 4 — XREF Sequence Check:
Canva exports text objects in reverse of creation order (reverse XREF sequence).
This creates a signature: visual order (top-to-bottom) is opposite to stream order
(object creation order). Spearman correlation between these two orderings detects this.

Pattern: When user edits a Canva-exported document (e.g., change amount in receipt),
Canva re-exports with the object order scrambled, leaving a detectable fingerprint.

Algorithm:
1. Extract all text objects with their XREF stream order
2. Sort by y-position to get visual order
3. Compute Spearman rank correlation between orderings
4. If correlation < -0.4: strong inverse (Canva signature) → xref_score = 50
5. If correlation < 0.0: weak inverse → xref_score = 25
6. If correlation < 0.2: near-random → xref_score = 10
7. Skip if producer/creator in whitelist (iText, reportlab, libreoffice, etc.)
"""
...
class XrefAnalyzer:
    ...
    def _extract_text_objects_with_order(self, page: fitz.Page) -> list[dict]:
        ...
        try:
            ...
            for stream_order, text in enumerate(trace):
