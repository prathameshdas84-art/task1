...
# ── Check 1: local variance / flat-zone (inpaint smoothing) ────────────────
...
# ── Check 2: glare vs digital-edit boundary discrimination ─────────────────
...
class ImageDocumentAnalyzer:
    ...
    def _flat_zone_check(self, gray, std_map, baseline):
        """Check 1: blocks whose local std collapsed relative to the
        document's own baseline; Check 2: each candidate region's boundary
        is classified sharp-step (digital edit) vs gradual (physical glare/
        ...
        for i in range(1, n_lbl):
            ...
            sub_lbl = np.zeros_like(sub_std, dtype=np.uint8)
    ...
    def _isolate_ink_regions(rgb):
        ...
        purple bands). Components are classified stamp (thick/solid) vs
        signature-like (thin pen stroke, via distance-transform thickness).
        Black/graphite ink has no color separation and is out of scope here."""
